...
// TestLoadAllStackConfigs_NoFile is the bootstrap path: a fresh ezstack
// home with no stacks.json yet must yield an empty map (not an error)
// so cross-repo discovery from a clean machine doesn't crash.
func TestLoadAllStackConfigs_NoFile(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("EZSTACK_HOME", tmp)

	all, err := LoadAllStackConfigs()
	if err != nil {
		t.Fatalf("unexpected error on missing stacks.json: %v", err)
	}
	if len(all) != 0 {
		t.Errorf("expected empty map, got %d entries: %v", len(all), all)
	}
}

// TestLoadAllStackConfigs_MultipleRepos is the headline test: every repo
// recorded in stacks.json comes back with hashes populated, branches
// loaded, and AgentSessionID readable on Stack — exactly what `ezs agent
// ls -a` walks.
func TestLoadAllStackConfigs_MultipleRepos(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("EZSTACK_HOME", tmp)

	file := stackConfigFile{
		...
		Repos: map[string]*repoData{
			"/repo/a": {
				...
				Branches: map[string]*BranchCache{
					"feat-a1": {AgentSessionID: "branch-uuid-a1"},
				},
			},
			"/repo/b": {
				Stacks: map[string]*Stack{
	...
	if err := os.WriteFile(filepath.Join(tmp, "stacks.json"), data, 0644); err != nil {
		t.Fatalf("write seed file: %v", err)
	}

	all, err := LoadAllStackConfigs()
	if err != nil {
		t.Fatalf("LoadAllStackConfigs: %v", err)
	}
	if len(all) != 2 {
	...
	repoA := all["/repo/a"]
	if repoA == nil {
		t.Fatal("missing /repo/a")
	}
	stackA := repoA.Stacks["abc1234"]
	if stackA == nil {
		t.Fatal("missing stack abc1234 in /repo/a")
	}
	if stackA.Hash != "abc1234" {
		t.Errorf("Hash not populated from map key, got %q", stackA.Hash)
	}
	if stackA.AgentSessionID != "uuid-a" {
		t.Errorf("AgentSessionID lost on stack, got %q", stackA.AgentSessionID)
	}
	if stackA.Name != "alpha" {
		t.Errorf("Name lost, got %q", stackA.Name)
	}
	if len(stackA.Branches) == 0 {
		t.Error("PopulateBranches not run — Branches slice is empty")
	}
	if bc := repoA.Cache.GetBranchCache("feat-a1"); bc == nil || bc.AgentSessionID != "branch-uuid-a1" {
		t.Errorf("branch cache AgentSessionID lost: %+v", bc)
	}

	repoB := all["/repo/b"]
	if repoB == nil {
		t.Fatal("missing /repo/b")
	}
	if repoB.Stacks["def5678"].AgentSessionID != "uuid-b" {
		t.Errorf("repo B AgentSessionID lost")
	}
}

// TestLoadAllStackConfigs_OldVersionMigrates verifies that an out-of-date
// stacks.json gets migrated by the LoadStackConfig("") side-effect call
// inside LoadAllStackConfigs. Without that, sessions on an older schema
// would never reach the cross-repo view.
func TestLoadAllStackConfigs_OldVersionMigrates(t *testing.T) {
	tmp := t.TempDir()
	t.Setenv("EZSTACK_HOME", tmp)
  ...
  "repos": {
	...
	if err := os.WriteFile(filepath.Join(tmp, "stacks.json"), v1, 0644); err != nil {
		t.Fatal(err)
	}

	if _, err := LoadAllStackConfigs(); err != nil {
		t.Fatalf("LoadAllStackConfigs on v1 file: %v", err)
	}

	migrated, err := os.ReadFile(filepath.Join(tmp, "stacks.json"))
	if err != nil {
		t.Fatalf("re-read after migration: %v", err)
	}
	var version struct {
		Version int `json:"version"`
	}
	if err := json.Unmarshal(migrated, &version); err != nil {
		t.Fatalf("parse migrated file: %v", err)
	}
	if version.Version != currentStackConfigVersion {
		t.Errorf("file not migrated: version = %d, want %d", version.Version, currentStackConfigVersion)
	}
}
