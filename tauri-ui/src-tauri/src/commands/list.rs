...
#[tauri::command]
pub fn get_stacks_status(
    state: State<'_, ConnectionState>,
    repo_path: String,
) -> Result<Vec<StatusStack>, String> {
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let result = run_ezs(&repo_path, &["-y", "status", "--json", "--all"], conn.as_ref())?;
    if result.exit_code != 0 {
        return Err(format!(
            "ezs status failed (exit {}): {}",
...
pub fn get_repo_path(
    ...
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let result = run_git(&start_path, &["rev-parse", "--show-toplevel"], conn.as_ref())?;
    if result.exit_code != 0 {
...
pub fn get_current_branch(
    ...
    let conn = state.0.lock().map_err(|e| e.to_string())?;
    let result = run_git(&repo_path, &["rev-parse", "--abbrev-ref", "HEAD"], conn.as_ref())?;
    if result.exit_code != 0 {
